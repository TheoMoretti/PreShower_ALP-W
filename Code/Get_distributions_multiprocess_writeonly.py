...
#The code is run over different samples of couplings for limitation of the copmuting power (16Go of RAM), the number of elements in each list can be increased with better computing power or smaller value for the "nsample" variable.
if args.index == 1:
...
Adress="/data/Discovery_potential/Raw_ALP_Decay/"
...
def do_convolution(coupling):
    ...
    for p,w in zip(particles_llp,weights_llp):
        ...
        for icoup,coup in enumerate(self.couplings):
            #add event weight and compute the relevant quantities to the ALPs
